...
## 6) بناء ملف APK بدون تثبيت Flutter (GitHub Actions)

1. ارفع محتويات هذا المجلد (بما فيها `.github` و`lib` و`assets` و`test`) إلى مستودع GitHub.
2. تأكد أن الملف `.github/workflows/build-apk.yml` موجود (يمكنك تسميته `main.yml`).
3. من تبويب **Actions** شغّل **Build APK** (أو يبدأ تلقائيًا عند كل رفع).
4. بعد العلامة الخضراء نزّل `almegdaa-apk` من **Artifacts**، وفك الضغط، وثبّت `app-release.apk` على الجوال.

الملف يضيف تلقائيًا: صلاحية الكاميرا، اسم التطبيق، الأيقونة، وفرض compileSdk 36 على المكتبات.
